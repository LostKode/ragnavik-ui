...
| 1.2.0 | Add original Ragnavik startup and initial world entry loading screens using eight packaged images and 44 packaged tips. Randomize images and tips without immediate repetition, preserve image aspect ratio, replace the supported startup loading spinner with an animated textless Fjord Gate mark, and retain vanilla UI fallbacks when custom assets or UI objects are unavailable. Leave teleport loading screens under their existing owner. Correct the AzuClock dependency to the verified Hexium 1.1.1 release. |
