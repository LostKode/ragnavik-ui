...
| Version | Changes |
| --- | --- |
| 1.1.0 | Add a separate Ragnavik changelog button and panel without changing Valheim's changelog. Load the authoritative changelog from the Ragnavik website. Cache the last successful response for offline launches and temporary website failures. Add configuration for automatic opening and the public endpoint. Remove the direct CurrencyPocket dependency because Ragnavik UI no longer adjusts its layout. |
